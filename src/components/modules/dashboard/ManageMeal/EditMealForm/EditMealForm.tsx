"use client";
import { createMeal, updateMealById } from "@/actions/meal.action";
...
const EditMealForm = ({
  ...
  const defaultValues = {
    ...
    dietery_id: initialData?.dietery_id || "",
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const toastId = toast.loading("Updating a meal data");
      try {
        ...
        const result = await updateMealById(mealId, payloadData);
        console.log("result after form submission", result);
        if (result?.id) {
          toast.success("Successfully updated a Meal data", { id: toastId });
          setMealImage([]);
          router.push('/provider-dashboard/manage-meal');
        }
      } catch (error: any) {
  ...
  return (
    <Card className="w-full">
      {/* card header */}
      <CardHeader>
        <CardTitle>Edit The Meal</CardTitle>
        <CardDescription>
          Fill in the details below to Edit a meal
        </CardDescription>
      </CardHeader>
